<template>
  <div class="appointments-page">
    <div class="topbar">
      <button class="back-btn" @click="router.push('/psychologist')">← Voltar</button>
        ...
        <div v-for="appointment in appointments" :key="appointment.id" class="appointment-row">
          <div class="appointment-info">
            ...
            <div>
              ...
              <p>{{ appointment.patientName }} {{ appointment.patientLastName }} · {{ appointment.type === 'O' ? 'Online' : 'Presencial' }}</p>
            </div>
          </div>
          <div class="appointment-actions" v-if="appointment.status === 'pendente'">
            <button class="finish-btn" @click="finishAppointment(appointment)">Finalizar</button>
            <button class="cancel-btn" @click="cancelAppointment(appointment)">Cancelar</button>
          </div>
        </div>
      </div>
...
import { getPsychologistAppointments, updateAppointmentStatus, type AppointmentListDTO } from '@/services/api'
...
async function fetchAppointments() {
  ...
  try {
    ...
    appointments.value = psychologistId != null ? await getPsychologistAppointments(psychologistId) : []
  } catch (error) {
...
async function finishAppointment(appointment: AppointmentListDTO) {
  if (!confirm('Finalizar esta consulta?')) return
  const ok = await updateAppointmentStatus(appointment.id, 'Finalizado')
  if (ok) {
